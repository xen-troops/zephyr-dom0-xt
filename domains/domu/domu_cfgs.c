...
void create_and_unpause_domus(void)
{
	for (unsigned int i = 0; i < ARRAY_SIZE(domu_cfg); i++) {
		...
		if (!domid || domid == 1) {
			LOG_ERR("Can't get domain ID for name %s or it's incorrect", domu_cfg[i].name);
			continue;
		}

		ret = domain_create(&domu_cfg[i], domid);
		if (ret) {
			LOG_ERR("Can't create domain %s", domu_cfg[i].name);
			continue;
		}

		ret = domain_post_create(&domu_cfg[i], domid);;
		if (ret) {
			LOG_ERR("Can't post create domain %s", domu_cfg[i].name);
